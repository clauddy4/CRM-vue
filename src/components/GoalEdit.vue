...
export default {
  ...
  methods: {
    async submitHandler() {
      ...
      try {
        ...
        this.$message(localizeFilter('Goal_HasBeenUpdated'))
        this.$emit('updated', goalData)
      } catch (e) {}
    },
  },
  created() {
    const { id, title, limit } = this.goals[0]
    this.current = id
    this.title = title
    this.limit = limit
  },
  ...
  watch: {
    current(goalId) {
      const { title, limit } = this.goals.find((g) => g.id === goalId)
      this.title = title
      this.limit = limit
    },
  },
